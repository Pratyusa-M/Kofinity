...
const BankSelect = ({ data, placeholder, getSelectedOpt, setId,passId,passOpt }) => {
  ...
  return (
    <div className={`relative h-[48px]`} ref={dropdownRef}>
      ...
      {isDropdownOpen && (
        <div className="absolute top-[40px] rounded pt-0 bg-[#2E2E2E] w-[100%] h-auto  flex flex-col z-30 overflow-y-auto max-h-[120px] ff-scroll">
          {data?.map((each) => (
            <p
              key={each.id}
