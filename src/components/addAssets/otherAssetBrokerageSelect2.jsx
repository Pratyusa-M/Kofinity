...
const OtherAssetBrokerageSelect2 = ({ data, placeholder, getSelectedOpt, setId,passId,passOpt,dataLength,sCode }) => {
...
//   const [selectedItemId, setSelectedItemId] = useState(data[0]?.id);
  ...
  return (
    <div className={`relative h-[48px]`} ref={dropdownRef}>
      ...
      {isDropdownOpen && (
        <div className="absolute top-[40px] rounded pt-0 bg-[#2E2E2E] w-[100%] h-auto  flex flex-col overflow-y-auto max-h-[120px] ff-scroll z-20">
          {data?.map((each,idx) => (
            <p
              key={idx}
...
export default OtherAssetBrokerageSelect2;
